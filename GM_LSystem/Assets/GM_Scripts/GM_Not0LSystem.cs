...
public class GM_Not0LSystem : MonoBehaviour
{
    //Members
    ...
    public void OnGenerator()
    {
        ...
        foreach (char i in st_Rule1)
        {
            ...
            //Note that the distance is half because this works better with L-Systems in which the Fs are not next to each other (Ex: F+F+F--F). This improves the quality of the image.
            if (ch_Axiom1 == i)
            {
                gameObject.transform.position += transform.up / 2;
                GameObject go_Temp;
